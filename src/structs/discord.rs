...
pub async fn auth_code(client: &mut reqwest::Client, app: &DiscordApp, code: String) -> Option<OAuthToken> {
    let req = client.post(URL.to_owned() + "/oauth2/token")
        .form(&[
...
pub async fn get_user(client: &mut reqwest::Client, auth: String, user: &str) -> Option<DiscordUser> {
    let req = client.get(URL.to_owned() + "/users/" + &user)
