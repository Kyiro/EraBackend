use crate::structs::{ALPHABET, app::*, discord::*};
...
pub async fn discord_oauth(
    ...
    let auth = match auth_code(&mut client, &app.discord, query.code.clone()).await {
        Some(d) => d, None => return HttpResponse::InternalServerError().into()
    };
    let user = match get_user(&mut client, auth.access_token, "@me").await {
        Some(d) => d, None => return HttpResponse::InternalServerError().into()
    };
    
    if let None = app.database.users.find_one(doc! { "discord_id": user.id.clone() }, None).await.unwrap() {
        ...
        app.database.athena.insert_one(Athena::new(account_id.clone()), None).await.unwrap();
        app.database.cloudstorage.insert_one(CloudStorage::new(account_id.clone()), None).await.unwrap();
    }
